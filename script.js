...
function renderGiftCard(gift) {
  ...
  const note = gift.note ? `<p class="gift-note">${escapeHtml(gift.note)}</p>` : '';
  return `<article class="gift-card"><div class="photo-frame">${photoMarkup}${controls}</div><div class="card-content"><h3>${escapeHtml(gift.name || 'Regalo senza nome')}</h3><p class="price">${formatPrice(gift.price)}</p>${note}${links ? `<div class="store-links">${links}</div>` : ''}<button class="primary-button" type="button" data-action="reserve" data-id="${escapeAttribute(gift.id)}" ${gift.reserved ? 'disabled' : ''}>${reservedByMe ? 'Scelto da te' : gift.reserved ? 'Già scelto' : 'Scegli questo regalo'}</button></div></article>`;
}
